#!/usr/bin/env bash
# shellcheck disable=SC2154  # GREEN, NC, ICON defined in initialize.sh
...
# Directory handling: arg or auto-detect
DIRECTORY_PATH=""
USER_PROVIDED_PATH=""
...
open_random_video() {
  if [[ ! -d "${DIRECTORY_PATH}" ]]; then
    ...
    echo "Please update the DIRECTORY_PATH variable."
    return 1
  fi
  ...
  while IFS= read -r -d '' file; do
    # Skip if in played set and saving enabled
    if [[ -n "${args[save_played]}" ]] && [[ -n "${PLAYED_SET["${file}"]}" ]]; then
      continue
    fi
    videos+=("${file}")
